...
use task_hookrs::task::{TW26, Task};
...
fn parse_and_render(original: &str, modified: &str) -> String {
    let mut modified_task: Task<TW26> = import_task(modified).expect(modified);
    ...
    let original_task: Task<TW26> = import_task(original).unwrap();
    ...
    println!("Not completing task - just rescheduling");
...
mod tests {
    ...
    fn test_reschedule() {
        ...
        let mut expect_task: Task<TW26> = import_task(modified).unwrap();
        let new_scheduled = today_in_utc() + chrono::Duration::days(1);
        expect_task.set_scheduled(Some(new_scheduled));

        let result_task: Task<TW26> =
            import_task(parse_and_render(&original, &modified).as_ref()).unwrap();
